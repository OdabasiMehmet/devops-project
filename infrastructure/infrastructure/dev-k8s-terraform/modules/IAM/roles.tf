resource "aws_iam_policy" "policy_for_master_role" {
  ...
  policy      = file("./modules/IAM/policy_for_master.json") // We will get the role from the file we created
}

resource "aws_iam_policy" "policy_for_worker_role" {
  name        = var.worker-role-policy
  policy      = file("./modules/IAM/policy_for_worker.json") // We will get the role from the file we created
}
